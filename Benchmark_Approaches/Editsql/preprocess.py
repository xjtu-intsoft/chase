...
def write_interaction(interaction_list,split,output_dir):
  ...
  with open(json_split, 'w') as outfile:
    for interaction in interaction_list:
      json.dump(interaction, outfile, indent=4, ensure_ascii=False)
      outfile.write('\n')
...
def remove_from_with_join(format_sql_2):
    ...
    if 'select ' in sub_sql:
      ...
      for t_i, t in enumerate(sub_sql_tokens):
        if t == 'as':
          ...
          tables_to_name[sub_sql_tokens[t_i + 1]] = sub_sql_tokens[t_i - 1]
  ...
  for i in range(len(format_sql_3)):
    for table, name in tables_to_name.items():
      if format_sql_3[i] != format_sql_3[i].replace(table, name):
        print(format_sql_3)
        format_sql_3[i] = format_sql_3[i].replace(table, name)
...
def read_data_json(split_json, interaction_list, database_schemas, column_names, output_vocab, schema_tokens, remove_from):
  ...
  for interaction_data in split_data:
    interaction_data = json.loads(json.dumps(interaction_data, ensure_ascii=False))
    valid = True
    ...
    for turn in interaction_data['interaction']:
      ...
      # TODO Correct some human sql annotation error
    ...
    if len(interaction['interaction']) > 0 and valid:
      interaction_list[db_id].append(interaction)
...
def read_chase(chase_dir, database_schemas, column_names, output_vocab, schema_tokens, remove_from):
  interaction_list = {}

  train_json = os.path.join(chase_dir, 'train.json')
  interaction_list = read_data_json(train_json, interaction_list, database_schemas, column_names, output_vocab, schema_tokens, remove_from)

  dev_json = os.path.join(chase_dir, 'dev.json')
  interaction_list = read_data_json(dev_json, interaction_list, database_schemas, column_names, output_vocab, schema_tokens, remove_from)
...
def preprocess(dataset, remove_from=False):
  ...
  if dataset == 'spider':
    ...
    output_dir = 'data/chase_data'
    if remove_from:
      output_dir = 'data/chase_data_removefrom'
    train_database, dev_database = read_db_split(chase_dir)
  ...
  print(f'Reading {dataset} database schema file')
  ...
  with open(output_database_schema_filename, 'w') as outfile:
    json.dump([v for k,v in database_schemas.items()], outfile, indent=4, ensure_ascii=False)
  ...
  if dataset == 'spider':
    ...
    interaction_list = read_chase(chase_dir, database_schemas, column_names, output_vocab, schema_tokens, remove_from)
...
if __name__ == '__main__':
  ...
  parser.add_argument("--dataset", choices=('spider', 'sparc', 'cosql', 'chase'), default='chase')
