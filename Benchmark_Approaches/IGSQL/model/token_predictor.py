...
class SchemaTokenPredictor(TokenPredictor):
    ...
    def forward(self, prediction_input, dropout_amount=0.):
        ...
        intermediate_state = self._get_intermediate_state(state_and_attn, dropout_amount=dropout_amount)
        copy_score = torch.sigmoid(torch_utils.linear_layer(intermediate_state, self.state2copy_transform_weights).squeeze(0))
        vocab_scores, vocab_tokens = self._score_vocabulary_tokens(intermediate_state)
        ...
        final_scores = torch.cat([copy_score[0]*final_scores, copy_score[1]*schema_scores], dim=0)
        ...
        if query_scores is not None:
            final_scores = torch.cat([final_scores, copy_score[2]*query_scores.unsqueeze(1)], dim=0)
