...
from .bert_cn import tokenization as tokenization
from .bert_cn.modeling import BertConfig, BertModel

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

def get_bert(params):
    BERT_PT_PATH = './model/bert_cn/data/annotated_wikisql_and_PyTorch_bert_param'
    ...
    bert_config_file = os.path.join(BERT_PT_PATH, 'bert_config_{0}.json'.format(bert_type))
    vocab_file = os.path.join(BERT_PT_PATH, 'vocab_{0}.txt'.format(bert_type))
    init_checkpoint = os.path.join(BERT_PT_PATH, 'pytorch_model_{0}.bin'.format(bert_type))
