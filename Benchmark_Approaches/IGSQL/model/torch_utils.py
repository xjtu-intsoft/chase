...
def compute_loss(gold_seq,
    ...
    for i, gold_tok in enumerate(gold_seq):
        ...
        probdist = score
        prob_of_tok = torch.sum(probdist[gold_indices])
...
def add_params(size, name=""):
    ...
    if len(size) == 1:
        #return torch.nn.Parameter(torch.zeros(size_int))
        return torch.nn.Parameter(torch.empty(size_int).uniform_(-0.1, 0.1))
    else:
        tmp_ret = torch.empty(size_int)
        torch.nn.init.xavier_uniform_(tmp_ret)
        return torch.nn.Parameter(tmp_ret)
